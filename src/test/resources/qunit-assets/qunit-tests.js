...
test('getMessage', function() {
	...
	strictEqual("You have successfully deployed a Java EE 6 web application.",
			culturesService.getMessage("home_msg_1", "en"));
	strictEqual("Vous avez réussi à déployer une Java EE 6 web application.",
			culturesService.getMessage("home_msg_1", "fr"));
});
...
test('getMessages', function() {
	...
	ok(messagesEN !== undefined
			&& messagesEN.home_msg_1 === "You have successfully deployed a Java EE 6 web application.",
				"Messages are in English");
	var messagesFR = culturesService.getMessages("fr");
	ok(messagesFR !== undefined
			&& messagesFR.home_msg_1 === "Vous avez réussi à déployer une Java EE 6 web application.",
...
test('MainCtrl', function() {
	...
	ok(messages !== undefined
			&& messages.home_msg_1 === "You have successfully deployed a Java EE 6 web application.",
				"Messages are in English");
	strictEqual("en", Globalize.culture().name);
	this.$scope.changeCulture("fr");
	messages = this.$scope.messages;
	ok(messages !== undefined
			&& messages.home_msg_1 === "Vous avez réussi à déployer une Java EE 6 web application.",
				"Messages are in French");
	strictEqual("fr", Globalize.culture().name);

	// refresh method test
	this.$scope.newMember = {
		"name" : "John",
